...
class BlockSeatsRestApiTest @Autowired constructor(val gateways: AxonGatewaysMock) {

    @Test
    fun `command success - returns 204 No Content`() {
        gateways.assumeCommandReturns<BlockSeats>(Success)

        Given {
            contentType(ContentType.JSON)
            body("""{"seats": ["1:1", "1:2"], "blockadeOwner": "Reservation:123"}""")
        } When {
            async().post("/screenings/screening-1/seats-blockades")
        } Then {
    ...
    fun `command failure - returns 400 Bad Request`() {
        gateways.assumeCommandReturns<BlockSeats>(Failure("Cannot block seats - some seats are already blocked by others: [1:1]"))
        ...
        Given {
            ...
            contentType(ContentType.JSON)
            body("message", equalTo("Cannot block seats - some seats are already blocked by others: [1:1]"))
        }
    }
}
