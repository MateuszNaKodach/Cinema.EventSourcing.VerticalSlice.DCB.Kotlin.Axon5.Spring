...
private val usedDates: MutableSet<LocalDate> = Collections.synchronizedSet(mutableSetOf())
fun randomDate(): LocalDate {
    var date: LocalDate
    do {
        date = LocalDate.of(year, month, day).plusDays(generated.getAndIncrement().toLong())
    } while (!usedDates.add(date))
    return date
}
